...
export async function withoutResetCountHourlyData(req, res) {
  ...
  try {
    ...
    // 1. Filter data for the selected date and sort chronologically
    ...
    // 2. Extract the last Count value for each hour
    ...
    // 3. Calculate production across the 24-hour period
    for (let i = 0; i < 24; i++) {
      ...
      if (lastCountsByHour[hourStr] !== undefined) {
        ...
        if (currentLastCount >= previousLastCount) {
          ...
          // Add current count if a machine reset is detected
    ...
    // 4. Format data according to the shift hours array
...
export async function getProductionGaps(req, res) {
  ...
  try {
    ...
    const machineSnapshot = await get(ref(rtdb, `Machines/${machineId}`));
    const machineData = machineSnapshot.val() || {};

    const startTime = machineData.productionStartTime || "08:30";
    const endTime = machineData.productionEndTime || "20:30";
    const dailyTarget = Number(machineData.dailyTarget || 0);
    ...
    // Planned Average Gap Calculation
...
export const removeAssignment = async (req, res) => {
  // Restrict removal to Admin/Superuser for security
...
export const updateLineDetails = async (req, res) => {
  ...
  try {
    ...
    // 4. If Supervisor, verify if line belongs to them
    ...
    // 5. Update data (restricted updates based on previous assignments)
    await update(lineRef, {
...
export const getLiveDataByLineId = async (req, res) => {
  try {
    ...
    if (!machineId) {
      return res.status(404).json({
        success: false,
        message: "No machine assigned to this line",
      });
    }
...
// Esp32DataController.js

export async function getProductionGapsByLineId(req, res) {
  const { lineId } = req.params;
  const { date } = req.query;

  try {
    // 1. Line ID එක හරහා අදාළ Machine ID එක සොයා ගැනීම
    const lineSnapshot = await get(ref(rtdb, `Lines/${lineId}`));
    ...
    // 2. Machine ID එක භාවිතා කර CounterHistory ලබා ගැනීම
    const historySnapshot = await get(ref(rtdb, `Machines/${machineId}/CounterHistory`));
    ...
    // Line Data හෝ Machine Data වලින් අදාළ විස්තර ලබා ගැනීම
